...
const LiquidityInfo = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const liquidityInfoTop =
        liquidityInfoRef.current.getBoundingClientRect().top;
  ...
  return (
    <div id="about" className="relative w-full py-[40px] overflow-hidden">
      <img
        src={bgImage}
        alt="Hero Background"
        className="absolute opacity-65 top-0 left-0 w-full h-full object-cover"
      />
      <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-40"></div>
      <div
        onMouseEnter={() => showfullwidth(true)}
        onMouseLeave={() => showfullwidth(false)}
        className="mx-[9%] flex flex-col mb-[36px] w-auto relative "
      >
        <h1
          className={`font-anton tracking-widest transition-all duration-500 text-white font-bold text-[29px] md:text-[38px]`}
        >
          Achievements
        </h1>
        <span
      ...
      <div className="flex md:flex-row mx-[9%] flex-col justify-center gap-8 mb-8 items-center">
        ...
        <div
          ref={liquidityInfoRef}
          className="flex flex-col justify-evenly gap-8"
        >
          <div
